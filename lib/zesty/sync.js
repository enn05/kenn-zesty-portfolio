 ...
 */
 ...
 (async () => {
   ...
   const tasks = new Listr([
     ...
     
     {
       ...
       skip: (ctx) => {
         // Honour skip_config_overwrite here as well as on the settings task.
         // Upstream only skips when stage_password is non-empty, but ours is
         // deliberately blank -- the password lives in .env.local so it stays
         // out of a file committed to three remotes. Without this check the
         // task rewrites zesty.config.json from the template, which would
         // put the password back, downgrade production to http://, and reset
         // skip_config_overwrite to false.
         if (ctx.nextConfig?.options?.skip_config_overwrite === true) {
           return 'skip_config_overwrite is set in zesty.config.json';
         }
         if (
 ...
 async function createFiles(config) {
   ...
   // Fetch instance data.
   // zesty.config.json is committed with a blank stage_password, so prefer the
   // environment. Load .env.local manually — this script runs under plain node,
   // not through Next, so nothing has populated process.env for us.
   if (!process.env.ZESTY_STAGE_PASSWORD) {
     try {
       const envFile = require('fs').readFileSync(
         require('path').resolve(process.cwd(), '.env.local'),
         'utf8',
       );
       const hit = envFile.match(/^\s*ZESTY_STAGE_PASSWORD\s*=\s*(.+)\s*$/m);
       if (hit) process.env.ZESTY_STAGE_PASSWORD = hit[1].trim();
     } catch (err) {
       /* no .env.local — fall through to the config value */
     }
   }
   const zpw = process.env.ZESTY_STAGE_PASSWORD || config.stage_password || '';

   const gql = `${config.stage}/-/gql/?zpw=${zpw}`;
